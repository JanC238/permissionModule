...
<script src="/lsfb/permissionModel/Public/admin/js/jquery.min.1.8.1.js"></script>
</head>
<style>
*{margin:0 auto;padding:0;list-style-type:none;}
body{background-color: #2F85DF;}
.content{width: 1000px; height: 100%;  margin-top:10%; text-align: center;}
.inp_divone{ width: 314px; height: 44px;margin-bottom: 20px;background-image: url('/lsfb/permissionModel/Public/admin/main/border.png'); }
.true_login{ margin-top:30px; }
.right_yuan{ cursor:pointer; float: right;width: 40px; height: 40px; margin-top: 3px; margin-right: 7px; background-image: url('/lsfb/permissionModel/Public/admin/main/write.png');
	...
	$(".right_yuan").live("click",function(){
		$.post("<?php echo U('/AdAccount/login');?>",{"name":$("#name").val(),"pwd":$("#pwd").val()},function(data){
			if(data==1){
				...
				location.href="/lsfb/permissionModel/pic.php/Index/index";
			}
		})
	})	;
	//enter 登录
    	...
    	if (event.keyCode == 13) {
        	$.post("<?php echo U('/AdAccount/login');?>",{"name":$("#name").val(),"pwd":$("#pwd").val()},function(data){
				if(data==1){
					...
					window.location.href="/lsfb/permissionModel/pic.php/Index/index";
				}
			});
        }
    });
...
<body>
  <div class="content">
     <div><img src="/lsfb/permissionModel/Public/admin/main/logo.png" /></div>
